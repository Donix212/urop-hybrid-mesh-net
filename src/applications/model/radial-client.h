// /*
//  * Copyright (c) 2007,2008,2009 INRIA, UDCAST
//  *
//  * SPDX-License-Identifier: GPL-2.0-only
//  *
//  * Author: Ishaan Lagwankar <lagwanka@msu.edu>
//  *
//  */

// #ifndef RADIAL_CLIENT_H
// #define RADIAL_CLIENT_H

// #include "source-application.h"

// #include "ns3/deprecated.h"
// #include "ns3/event-id.h"
// #include "ns3/ipv4-address.h"
// #include "ns3/ptr.h"
// #include "ns3/traced-callback.h"
// #include "ns3/replay-clock.h"

// #include <optional>

// namespace ns3
// {

// class Socket;
// class Packet;

// /**
//  * @ingroup radialclientserver
//  *
//  * @brief A Radial client. Sends UDP packet carrying sequence number and time stamp
//  *  in their payloads
//  *
//  */
// class RadialClient : public SourceApplication
// {
//   public:
//     /**
//      * @brief Get the type ID.
//      * @return the object TypeId
//      */
//     static TypeId GetTypeId();

//     RadialClient();
//     ~RadialClient() override;

//     static constexpr uint16_t DEFAULT_PORT{100}; //!< default port

//     /**
//      * @brief set the remote address and port
//      * @param ip remote IP address
//      * @param port remote port
//      */
//     NS_DEPRECATED_3_44("Use SetRemote without port parameter instead")
//     void SetRemote(const Address& ip, uint16_t port);
//     void SetRemote(const Address& addr) override;

//     /**
//      * @return the total bytes sent by this app
//      */
//     uint64_t GetTotalTx() const;

//   private:
//     void StartApplication() override;
//     void StopApplication() override;

//     /**
//      * @brief Send a packet
//      */
//     void Send();

//     /**
//      * @brief Set the remote port (temporary function until deprecated attributes are removed)
//      * @param port remote port
//      */
//     void SetPort(uint16_t port);

//     /**
//      * @brief Get the remote port (temporary function until deprecated attributes are removed)
//      * @return the remote port
//      */
//     uint16_t GetPort() const;

//     /**
//      * @brief Get the remote address (temporary function until deprecated attributes are removed)
//      * @return the remote address
//      */
//     Address GetRemote() const;

//     /**
//      * @brief Get a random peer address from the peer list
//      * @return a random peer address
//      */
//     Address GetRandomPeer() const;

//     /**
//      * @brief Get the replay clock for packet timestamps
//      * @return the replay clock
//      */
//     ReplayClock GetReplayClock() const override;
    
    
//     uint32_t m_count; //!< Maximum number of packets the application will send
//     Time m_interval;  //!< Packet inter-send time
//     uint32_t m_size;  //!< Size of the sent packet (including the SeqTsHeader)
//     std::optional<uint16_t> m_peerPort; //!< Remote port (temporary until deprecated attributes are removed)
//     uint32_t m_sent;                    //!< Counter for sent packets
//     uint64_t m_totalTx;                 //!< Total bytes sent
//     Ptr<Socket> m_socket;               //!< Socket
//     EventId m_sendEvent;                //!< Event to send the next packet
    
//     vector<Address> m_peerList;         //!< List of remote addresses

//     ReplayClock m_replayClock;          //!< Replay clock for packet timestamps
// };

// } // namespace ns3

// #endif /* RADIAL_CLIENT_H */
