...
ReplayClock::MergeSameEpoch(ReplayClock o_replayClock, int64_t u_epsilon)
{
    ...
    while(bitmap > 0)
    {
        ...
        if(m_bitmap[pos] == 1 && o_replayClock.m_bitmap[pos] == 1)
        {
            ...
            new_offset = std::min(GetOffsetAtIndex(left_index, u_epsilon), o_replayClock.GetOffsetAtIndex(right_index, u_epsilon));
            ++left_index;
            ++right_index;
...
{
    NS_LOG_FUNCTION(this);
    if(o_replayClock.m_hlc != m_hlc || o_replayClock.m_bitmap != m_bitmap || o_replayClock.m_offsets != m_offsets)
    {
        return false;
    }
    return true;
