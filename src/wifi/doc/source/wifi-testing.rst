...
A verification and validation of rate control algorithms in |ns3| (``ns3::MinstrelHtWifiManager``,
``ns3::IdealWifiManager``, and ``ns3::ThompsonSamplingWifiManager``) was published in [leon]_.
If users want to run and/or modify these scripts please refer to the paper where a
link to the scripts is provided. In this work, the basic operation and convergence was evaluated
for a variety of parameter combinations.  Below, we also refer to these algorithms as rate
adaptation algorithms (RAAs).
...
and is similar to the ``wifi-manager-example.cc`` script in the |ns3| codebase.
The highest rate selected by each manager is then compared across possible configurations
and with published MCS tables. The |ns3| RAAs are able to select appropriate MCSes
...
statistics, both the |ns3| MinstrelHt and ThompsonSampling models require time to sample possible
...
``ns3::IdealWifiManager`` this test is not relevant since convergence is immediate due to
the perfect SNR feedback provided.

A key finding users should be aware of is that convergence time will vary depending on whether
the channel conditions can successfully support a specific MCS or not.
If the channel conditions allow for a higher MCS to be successful, but resulting in high packet
error ratio (PER), then it is not clear if the rate control should select such an MCS. This is
because there will be a tradeoff between channel occupancy, latency, and throughput
depending on whether a higher or lower MCS is chosen.  Each adaptive RAA contains heuristics
that drive the performance for channel conditions in which the best MCS to select is not clear.
