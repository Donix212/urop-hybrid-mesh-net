                     ...
                     Time mpduDuration)
{
    ...
    rxSignalInfo.snr = DbToRatio((rxInfo.second.signal - rxInfo.second.noise).to<double>());
    rxSignalInfo.rssi = rxInfo.second.signal.to<double>();
...
PhyEntity::EndReceivePayload(Ptr<Event> event)
{
    ...
    if (std::count(statusPerMpdu.cbegin(), statusPerMpdu.cend(), true))
    {
        ...
        rxSignalInfo.rssi =
            signalNoiseIt->second.signal.to<double>(); // same information for all MPDUs
                              ...
                              Time mpduDuration)
{
    ...
    signalNoise.signal = dBm_t{event->GetRxPower(channelWidthAndBand.second)};
    signalNoise.noise = dBm_t{event->GetRxPower(channelWidthAndBand.second) / snrPer.snr};
    if (GetRandomValue() > snrPer.per &&
...
PhyEntity::CanStartRx(Ptr<const WifiPpdu> ppdu) const
{
    ...
    const auto primaryWidth = ((static_cast<uint16_t>(channelWidth.to<double>()) % 20 == 0)
                                   ? MHz_t{20}
