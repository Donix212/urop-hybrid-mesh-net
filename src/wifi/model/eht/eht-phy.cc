...
const std::map<MHz_t, WifiChannelListType> ehtSecondaryChannels {
    {20_MHz, WIFI_CHANLIST_SECONDARY},
    {40_MHz, WIFI_CHANLIST_SECONDARY40},
    {80_MHz, WIFI_CHANLIST_SECONDARY80},
    {160_MHz, WIFI_CHANLIST_SECONDARY160},
};
...
EhtPhy::GetCcaIndicationOnSecondary(const Ptr<const WifiPpdu> ppdu)
{
    ...
    for (auto secondaryWidth : secondaryWidthsToCheck)
    {
        ...
        {
            const auto band = m_wifiPhy->GetBand(20_MHz, index);
...
EhtPhy::GetPer20MHzDurations(const Ptr<const WifiPpdu> ppdu)
{
     ...
     * subchannel.
     */
    if (m_wifiPhy->GetChannelWidth() < 40_MHz)
    {
        return {};
    }
    ...
    for (auto index : indices)
    {
        auto band = m_wifiPhy->GetBand(20_MHz, index);
        /**
         * A signal is present on the 20 MHz subchannel at or above a threshold of –62 dBm at the
         * receiver's antenna(s). The PHY shall indicate that the 20 MHz subchannel is busy a period
         * aCCATime after the signal starts and shall continue to indicate the 20 MHz subchannel is
        ...
        if (ppdu)
        {
            const MHz_t subchannelMinFreq =
                m_wifiPhy->GetFrequency() - (m_wifiPhy->GetChannelWidth() / 2) + (index * 20_MHz);
            const MHz_t subchannelMaxFreq = subchannelMinFreq + 20_MHz;
