...
void
InterferenceHelper::SetNoiseFigure(scalar_t value)
{
    m_noiseFigure = value;
}
                                                ...
                                                const WifiSpectrumBandInfo& band) const
{
    ...
    for (; it != niIt->second.end() && it->first < now; ++it)
    {
        ...
        if (std::abs(noiseInterference.to<double>()) < std::numeric_limits<double>::epsilon())
        {
    ...
    NS_ASSERT_MSG(noiseInterference >= Watt_t{0.0},
...
double
InterferenceHelper::CalculateChunkSuccessRate(scalar_t sinr,
                                              Time duration,
                                              WifiMode mode,
                                              ...
                                              WifiPpduField field) const
{
    ...
    const auto csr =
        m_errorRateModel->GetChunkSuccessRate(mode, txVector, sinr, nbits, m_numRxAntennas, field);
    return csr;
}

double
InterferenceHelper::CalculatePayloadChunkSuccessRate(scalar_t sinr,
                                                     Time duration,
                                                     const WifiTxVector& txVector,
