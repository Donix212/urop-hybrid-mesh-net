                                          ...
                                          MHz_t guardBandwidth)
{
    ...
    else
    {
        ...
        const auto bandwidth = Hz_t((channelWidth + (2 * guardBandwidth) + separationWidth));
        // For OFDM, the center subcarrier is null (at center frequency)
        auto numBands =
            static_cast<uint32_t>(std::ceil(static_cast<double>(bandwidth / carrierSpacing)));
        ...
        const auto startingFrequency = Hz_t(*minCenterFrequency) -
                                       (Hz_t(channelWidth) / (2 * centerFrequencies.size())) -
                                       Hz_t(guardBandwidth) - (carrierSpacing / 2);
                                                          ...
                                                          MHz_t guardBandwidth)
{
    ...
    auto nGuardBands = static_cast<uint32_t>(((Hz_t(2 * guardBandwidth)) / carrierSpacing) + 0.5);
    auto nAllocatedBands = static_cast<uint32_t>((Hz_t(channelWidth) / carrierSpacing) + 0.5);
    ...
    // psd units are Watt/Hz
    const auto psd = txPowerPerBand.to<double>() / (bit->fh - bit->fl);
                                                          ...
                                                          dBr_t lowestPoint)
{
    ...
    case 20:
        ...
        innerSlopeWidth =
            static_cast<uint32_t>((Hz_t{2e6} / carrierSpacing) + 0.5); // [-11;-9] & [9;11]
        break;
    case 10:
        carrierSpacing = Hz_t{156250};
        innerSlopeWidth =
            static_cast<uint32_t>((Hz_t{1e6} / carrierSpacing) + 0.5); // [-5.5;-4.5] & [4.5;5.5]
        break;
    case 5:
        carrierSpacing = Hz_t{78125};
        innerSlopeWidth =
            static_cast<uint32_t>((Hz_t{5e5} / carrierSpacing) + 0.5); // [-2.75;-2.5] & [2.5;2.75]
        break;
    default:
    ...
    NS_ASSERT_MSG(std::abs(txPower.to<double>() - Integral(*c)) < 1e-6, "Power allocation failed");
    return c;
}
    ...
    const std::vector<bool>& puncturedSubchannels)
{
    ...
    const auto nGuardBands =
        static_cast<uint32_t>(((Hz_t(2 * guardBandwidth)) / carrierSpacing) + 0.5);
    const auto nAllocatedBands = static_cast<uint32_t>((Hz_t(channelWidth) / carrierSpacing) + 0.5);
    const auto separationWidth =
        MHz_t{std::abs((centerFrequencies.back() - centerFrequencies.front()).to<double>())};
    const auto unallocatedWidth =
        separationWidth > MHz_t{0} ? (separationWidth - (channelWidth / 2)) : MHz_t{0};
    const auto nUnallocatedBands =
        static_cast<uint32_t>((Hz_t(unallocatedWidth) / carrierSpacing) + 0.5);
    ...
    std::size_t numSubcarriersPer20MHz = Hz_t(MHz_t{20}) / carrierSpacing;
    ...
    const auto puncturedSlopeWidth =
        static_cast<uint32_t>((Hz_t{500e3} / carrierSpacing) +
    ...
    const std::vector<bool>& puncturedSubchannels)
{
    ...
    const auto separationWidth =
        MHz_t{std::abs((centerFrequencies.back() - centerFrequencies.front()).to<double>())};
    const Hz_t unallocatedWidth =
        separationWidth > MHz_t{0} ? (separationWidth - Hz_t(channelWidth / 2)) : MHz_t{0};
    const auto nUnallocatedBands =
        static_cast<uint32_t>((Hz_t(unallocatedWidth) / carrierSpacing) + 0.5);
    const auto nAllocatedBands = static_cast<uint32_t>((Hz_t(channelWidth) / carrierSpacing) + 0.5);
    NS_ASSERT_MSG(c->GetSpectrumModel()->GetNumBands() ==
    ...
    // Prepare spectrum mask specific variables
    auto innerSlopeWidth = static_cast<uint32_t>(
        (Hz_t(MHz_t{1}) / carrierSpacing) + 0.5); // size in number of subcarriers of the inner band
    std::vector<std::vector<WifiSpectrumBandIndices>> subBandsPerSegment(
        centerFrequencies.size()); // list of data/pilot-containing subBands (sent at 0dBr)
    WifiSpectrumBandIndices maskBand(0, nAllocatedBands + nGuardBands + nUnallocatedBands);
    ...
    case 20:
        ...
        innerSlopeWidth = static_cast<uint32_t>((Hz_t{5e5} / carrierSpacing) +
    ...
    case 320: // 4 x 80 MHz
        ...
        txPowerPerBand = txPower / (996 * (channelWidth.to<double>() / 80));
        skippedSubbands = 12;
    ...
    {
        for (std::size_t i = 1; i < (channelWidth / MHz_t{80}).to<double>(); ++i)
        {
    ...
    auto puncturedSlopeWidth =
        static_cast<uint32_t>((Hz_t{500e3} / carrierSpacing) +
                              0.5); // size in number of subcarriers of the punctured slope band
    std::vector<std::vector<WifiSpectrumBandIndices>> puncturedBandsPerSegment;
    std::size_t subcarriersPerSuband = (Hz_t(MHz_t{20}) / carrierSpacing);
    start = (nGuardBands / 2);
    ...
    uint32_t puncturedSlopeWidth)
{
    ...
    dBm_t txPowerRef{txPowerPerBand};
    ...
    // Different slopes
    double innerSlope = (-1.0 * minInnerBand.to<double>()) / innerSlopeWidth;
    double middleSlope =
        (-1.0 * (minOuterBand.to<double>() - minInnerBand.to<double>())) / middleSlopeWidth;
    double outerSlope = (txPowerMiddleBandMin - txPowerOuterBandMin).to<double>() / outerSlopeWidth;
    double puncturedSlope = (-1.0 * minInnerBand.to<double>()) / puncturedSlopeWidth;
    ...
    for (size_t i = 0; i < numBands; ++i)
    {
                 ...
                 (i <= betweenPsdsBand->second && i >= betweenPsdsBand->first))
        {
            ...
            {
                txPowerWPsds.at(1) = Watt_t{txPowerOuterBandMin +
                                            dB_t{(i - outerBandsLeft.at(1).first) * outerSlope}};
            }
            else if (i <= middleBandsLeft.at(1).second && i >= middleBandsLeft.at(1).first)
            {
                txPowerWPsds.at(1) = Watt_t{txPowerMiddleBandMin +
                                            dB_t{(i - middleBandsLeft.at(1).first) * middleSlope}};
            }
            else
            {
                NS_ASSERT(false);
            ...
            txPower = std::accumulate(txPowerWPsds.cbegin(), txPowerWPsds.cend(), Watt_t{0.0});
            txPower = std::max(Watt_t{txPowerRef - dB_t{25.0}}, txPower);
            txPower = std::min(Watt_t{txPowerRef - dB_t{20.0}}, txPower);
        }
        ...
        {
            txPower = Watt_t{txPowerOuterBandMin +
                             dB_t{(i - outerBandsLeft.at(psdIndex).first) * outerSlope}};
        }
        else if (i <= middleBandsLeft.at(psdIndex).second &&
                 i >= middleBandsLeft.at(psdIndex).first)
        {
            txPower = Watt_t{txPowerMiddleBandMin +
                             dB_t{(i - middleBandsLeft.at(psdIndex).first) * middleSlope}};
        }
        ...
        {
            txPower = Watt_t{txPowerInnerBandMin};
        }
            ...
            txPower = (!puncturedBandsPerSegment.empty() &&
                       ...
                       (puncturedBandsPerSegment.at(psdIndex).front().first <=
                        allocatedSubBandsPerSegment.at(psdIndex).front().first))
                          ...
                          Watt_t{txPowerInnerBandMin +
                                 dB_t{(i - innerBandsLeft.at(psdIndex).first) * innerSlope}};
        }
                                                                                ...
                                                                                // allocated band
        {
            ...
            if (insideSubBand)
            {
                ...
                if (insidePuncturedSubBand)
                {
                    ...
                    {
                        txPower = Watt_t{txPowerInnerBandMin +
                                         dB_t{(i - startPuncturedSlope) * puncturedSlope}};
                    }
                    else
                    {
                        txPower = std::max(
                            Watt_t{txPowerInnerBandMin},
                            Watt_t{txPowerRef -
                                   dB_t{(i - puncturedBandsPerSegment.at(psdIndex).at(0).first) *
                                        puncturedSlope}});
                    }
                }
                else
                 ...
                 i >= innerBandsRight.at(psdIndex).first)
        {
            ...
            txPower = std::min(
                previousTxPower,
                Watt_t{txPowerRef - dB_t{(i - innerBandsRight.at(psdIndex).first + 1) *
        ...
        {
            txPower = Watt_t{txPowerInnerBandMin -
                             dB_t{(i - middleBandsRight.at(psdIndex).first + 1) *
                                  middleSlope}}; // +1 so as to be symmetric with left slope
        }
        ...
        {
            txPower = Watt_t{txPowerMiddleBandMin -
                             dB_t{(i - outerBandsRight.at(psdIndex).first + 1) *
                                  outerSlope}}; // +1 so as to be symmetric with left slope
        }
        else
        {
        ...
        NS_LOG_LOGIC(i << " -> " << txPower.to<double>() / txPowerPerBand.to<double>());
    ...
    {
        *vit = txPowerValue.to<double>() * invBandwidth;
        vit++;
        bit++;
    }
                                       ...
                                       const std::vector<WifiSpectrumBandIndices>& segments)
{
    ...
    NS_ASSERT_MSG(power.to<double>() >= 0.0, "Invalid calculated power " << power);
    return power;
}
