...
SpectrumWifiPhyFilterTest::RxCallback(Ptr<const Packet> p, RxPowerWattPerChannelBand rxPowersW)
{
    ...
    dBm_t totalRxPowerDbm{it->second};
    NS_LOG_INFO("powerW total band: " << it->second << " (" << totalRxPowerDbm << ")");
    ...
    if ((m_txChannelWidth <= m_rxChannelWidth) && (channelWidth >= MHz_u{20}))
    {
        ...
        NS_LOG_INFO("powerW in primary 20 MHz channel: " << it->second << " ("
                                                         << dBm_t{it->second}.to<double>() << ")");
        const auto rxPowerPrimaryChannel20 = static_cast<int>(dBm_t{it->second}.to<double>() + 0.5);
