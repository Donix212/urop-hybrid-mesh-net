...
FromRss(dBm_u rss)
{
    ...
    dBm_t noisePower(-100.5522786 + 7);

    dB_t sinr = dBm_t{rss} - noisePower;
    // return SINR expressed as ratio
    return scalar_t{sinr};
}
...
TableBasedErrorRateTestCase::DoRun()
{
    ...
    for (dB_t snr = -4; snr <= dB_t{30}; snr += dB_t{0.25})
    {
        ...
        if (m_mode.GetMcsValue() > ERROR_TABLE_BCC_MAX_NUM_MCS)
        {
            ...
            expectedValue =
                1 - yans->GetChunkSuccessRate(m_mode, txVector, scalar_t{snr}, m_size * 8);
        }
        else
        {
        ...
        const auto per =
            1 - table->GetChunkSuccessRate(m_mode, txVector, scalar_t{snr}, m_size * 8);
