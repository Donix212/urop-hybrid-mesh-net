...
class SPFVertex
{
  public:
    ...
    typedef std::pair<Ipv4Address, int32_t>
        NodeExitv4_t; //!< IPv4 / interface container for exit nodes.

    typedef std::pair<Ipv6Address, int32_t>
        NodeExitv6_t; //!< IPv6 / interface container for exit nodes.
     ...
     * forwarding packets to the host or network represented by "this" SPFVertex.
     */
    void SetRootExitDirection(SPFVertex::NodeExitv6_t exit);
     ...
     * is out-of-range
     */
    NodeExitv6_t GetRootExitDirectionv6(uint32_t i) const;
     ...
     * 'this' vertex from the root
     */
    NodeExitv6_t GetRootExitDirectionv6() const;
     ...
     * @return The number of exit directions from root
     */
    uint32_t GetNRootExitDirectionsv6() const;
  ...
  private:
    ...
    Ipv4Address m_nextHopv4;                            //!< next hop for ipv4
    Ipv6Address m_nextHopv6;                            //!< next hop for ipv6
    typedef std::list<NodeExitv4_t> ListOfNodeExitv4_t; //!< container of Exit nodes
    typedef std::list<NodeExitv6_t> ListOfNodeExitv6_t; //!< container of Exit nodes

    ListOfNodeExitv4_t m_ecmpRootExitsv4; //!< store the multiple root's exits for supporting ECMP
    ListOfNodeExitv6_t m_ecmpRootExitsv6; //!< store the multiple root's exits for supporting ECMP
...
class GlobalRouteManagerLSDB
{
  ...
  private:
    ...
    static bool m_isIpv4; //!< Flag to determine if route is for Ipv4 or Ipv6 information
};
...
class GlobalRouteManagerImpl
{
  ...
  private:
     ...
     *
     * @param a the target IP address
     * @param aprefix the target subnet prefix
     * @return the outgoing interface number
     */
