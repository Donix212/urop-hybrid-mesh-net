...
This chapter defines the implementation of the ``GlobalRouting`` API present in the internet module.
...
To use GlobalRouting as the routing protocol,
an instance of Ipv4GlobalRoutingHelper needs to be set using InternetStackHelper::SetRoutingProtocol().
...
To run GlobalRouting and Populate Routing tables , functions in two interfaces namely,
Ipv4GlobalRoutingHelper and GlobalRouteManager can be used.

Helpers
~~~~~~~
Users must include ns3/ipv4-global-routing-helper.h header file.
After IP addresses are configured, the
...
.. note::
    ...
    wireless effects into account. For wireless, we recommend OLSR dynamic routing described below.
...
using the public Ipv4 API of that node.
...
Currently, the implementation can handle IPv4 point-to-point, numbered links,
...
No Formal Validation has been done for the Global Routing API.
