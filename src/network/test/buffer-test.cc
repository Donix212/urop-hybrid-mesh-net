...
class BufferTest : public TestCase
{
  private:
     ...
     */
    void EnsureWrittenBytes(Buffer b, const std::vector<uint8_t>& array);
...
void
BufferTest::EnsureWrittenBytes(Buffer b, const std::vector<uint8_t>& array)
{
    bool success = true;
    const std::vector<uint8_t>& expected = array;
    const uint8_t* got;
    got = b.PeekData();
    ...
    if (!success)
    {
        ...
        for (auto e : expected)
        {
            failure << static_cast<uint16_t>(e) << " ";
        }
