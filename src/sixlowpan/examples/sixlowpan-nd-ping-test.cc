 ...
 *  n6---------n7---------n8
 *
 * ./waf --run "scratch/udp-test.cc --Mesh --Position=Grid NumberOfNodes=9 --StopTime=200
 * --Interval=1"
 *
 */
...
// It can NOT tell you shit about the real packet size after compression, if there has been a
// fragmentation, etc. for that you need the function above.

void
SixLowCallback(std::string path,
               Ptr<const Packet> packet,
               Ptr<SixLowPanNetDevice> netDev,
               uint32_t index)
{
    Ipv6Header ipv6Hdr;
    Ptr<Packet> pktCpy = packet->Copy();
    ...
    // std::cout << Now ().As (Time::S) << " ******* "<< packet->GetSize () << " ******* " <<
    // *packet << std::endl;
}
...
main(int argc, char** argv)
{
  ...
  LogComponentEnable ("SixLowPanNetDevice", LOG_LEVEL_ALL);
    ...
    // this means that the registration is valid for 2 days (and the re-registration is performed
    // after 1 day).
    Config::SetDefault("ns3::SixLowPanNdProtocol::RegistrationLifeTime", UintegerValue(2880));
    // Config::SetDefault ("ns3::Icmpv6L4Protocol::SolicitationJitter", StringValue
    ...
    Config::Connect("/NodeList/*/DeviceList/*/$ns3::lrwpan::LrWpanNetDevice/Phy/PhyTxBegin",
