...
//         Ptr<Ipv6> ipv6b = nodesAB.Get(1)->GetObject<Ipv6>();
//         ipv6b->SetForwarding(2, true); // interface 1 = BC
//         ipv6b->SetForwarding(1, true); // interface 0 = AB

//         // A: default route via B (AB link)
//         Ptr<Ipv6StaticRouting> Astatic = DynamicCast<Ipv6StaticRouting>(
//             nodesAB.Get(0)->GetObject<Ipv6L3Protocol>()->GetRoutingProtocol());
//         Astatic->SetDefaultRoute(addrB, 1); // Interface to B

//         // B: route to A via AB, route to C via BC
//         Ptr<Ipv6StaticRouting> Bstatic = DynamicCast<Ipv6StaticRouting>(
//             nodesAB.Get(1)->GetObject<Ipv6L3Protocol>()->GetRoutingProtocol());
//         Bstatic->AddHostRouteTo(addrA, LLaddrA, 1);
//         Bstatic->SetDefaultRoute(addrC, 2);

//         // C: default route via B
//         Ptr<Ipv6StaticRouting> Cstatic = DynamicCast<Ipv6StaticRouting>(
//             nodesBC.Get(1)->GetObject<Ipv6L3Protocol>()->GetRoutingProtocol());
//         Cstatic->AddHostRouteTo(addrB2, LLaddrB2, 1);

//         std::ostringstream ndiscStream;
//         Ptr<OutputStreamWrapper> outputNdiscStream = Create<OutputStreamWrapper>(&ndiscStream);
//         Ipv6RoutingHelper::PrintNeighborCacheAllAt(Seconds(30), outputNdiscStream);
...
//         Ipv6RoutingHelper::PrintRoutingTableAllAt(Seconds(30), outputRoutingTableStream);

//         Time startTime = Seconds(5.0);
//         Time stopTime = Seconds(15.0);

//         auto InstallPing =
//             [](Ptr<Node> srcNode, Ipv6Address srcAddr, Ipv6Address dstAddr, Time start, Time
//             stop) {
//                 PingHelper ping6;
...
//         // A → B, C
//         InstallPing(A, addrA, addrB, startTime, stopTime); // works
//         InstallPing(A, addrA, addrC, startTime, stopTime);

//         // B → A, C
//         InstallPing(B, addrB, addrA, startTime, stopTime);  // works
//         InstallPing(B, addrB2, addrC, startTime, stopTime); // works

//         // C → A, B
//         // InstallPing(C, addrC, addrA, startTime, stopTime);
//         InstallPing(C, addrC, addrB2, startTime, stopTime); // works
