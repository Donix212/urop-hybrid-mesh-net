...
{
  // std::cout << Now ().As (Time::S) << " Sending NS(EARO) addrToRegister :" << addrToRegister << std::endl;
...
{
  // std::cout << Now ().As (Time::S) << " Sending NA(EARO) addrToRegister : " << target<< std::endl;
                              ...
                              Ptr<Ipv6Interface> interface)
{
  ...
  Ptr<SixLowPanNetDevice> sixDevice = DynamicCast<SixLowPanNetDevice> (interface->GetDevice ());
  NS_ASSERT_MSG (sixDevice, "SixLowPanNdProtocol cannot be installed on device different from SixLowPanNetDevice");
  ...
  //      HandleSixLowPanDAC (packet, header.GetSource (), header.GetDestination (), interface);
  //      break;
      default:
  ...
  if (!IsAddressRegistrationInProgress ())
    {
      ...
      // std::cout << Simulator::Now ().As (Time::MS) << " - " << m_node->GetId () << " - calling AddressRegistration (0)" << std::endl;
...
SixLowPanNdProtocol::AddressRegistration (void)
{
  ...
  if (m_addrPendingReg.isValid == false)
      ...
      if (!m_pendingRas.empty () && m_registeredAddresses.empty ())
      ...
      else if (!m_pendingRas.empty () && !m_registeredAddresses.empty ())
        {
          ...
          if (m_registeredAddresses.front ().registrationTimeout - Minutes (m_regTime)/2 <= Now ())
            {
              NS_LOG_LOGIC ("Address Registration: found an address that needs urgently a re-registration");
              addressPendingRegistrationIsNew = false;
              SixLowPanNdProtocol::AddressReRegistration ();
            }
          else
        ...
        {
          NS_ABORT_MSG ("SixLowPanNdProtocol::Address Registration called but no address to register - error.");
        }

      if (addressPendingRegistrationIsNew == true)
        // pendingRA and address is not registered, move data from pendingRA list to  and address being registered struct
        {
        ...
        {
          // No pendingRAs, move data from SixLowPanRegisteredAddress list to address being registered struct
          m_addrPendingReg.isValid = true;
  ...
  else
    {
      addressToRegister = m_addrPendingReg.addressPendingRegistration;
      Ipv6Address registeringAddressNodeAddr = addressToRegister.IsLinkLocal () ? m_addrPendingReg.registrar : m_addrPendingReg.abroAddress;
      auto it = m_tidContainer.find (std::make_pair (addressToRegister, registeringAddressNodeAddr));
...
//  std::cout << "+++++ Sending a NS(EARO) for " << addressToRegister << std::endl;
  ...
  // std::cout << "++AddressRegistration called for m_node= " << m_node->GetId () << " m_regTime = " << m_regTime
  ...
  if (m_addressRegistrationTimeoutEvent.IsRunning ())
     std::cout << "** FUUUUUCK" << std::endl;
       ...
       m_addressRegistrationTimeoutEvent = Simulator::Schedule (
           m_retransmissionTime, &SixLowPanNdProtocol::AddressRegistrationTimeout, this);
     return;
...
{
  // std::cout << "Address Registration Success Called for m_node= " << m_node->GetId () << " m_regTime = " << m_regTime
...
//    }
  ...
  if (m_addrPendingReg.newRegistration)
    {
      // std::cout << "********************this pecice of code runing here*********************************" << std::endl;
  ...
  if (m_addressRegistrationTimeoutEvent.IsRunning ())
    {
      // std::cout << "WARNING - we are skipping something because m_addressRegistrationTimeoutEvent is in progress - detais are:" << std::endl;
      // std::cout << "  m_addrPendingReg.newRegistration is " << m_addrPendingReg.newRegistration << std::endl;
      // std::cout << "  m_addrPendingReg.addressPendingRegistration is " << m_addrPendingReg.addressPendingRegistration << std::endl;
      // std::cout << "  registrar is                                   " << registrar << std::endl;
      // std::cout << " the list of this is:" << std::endl;
            for (auto& i : m_registeredAddresses)
        {
  ...
  if (!m_pendingRas.empty ())
    {
      ...
      // std::cout << Simulator::Now ().As (Time::MS) << " - " << m_node->GetId () << " - calling AddressRegistration (1)" << std::endl;
      m_addressRegistrationEvent = Simulator::Schedule (MilliSeconds (m_addressRegistrationJitter->GetValue ()),
                                                        &SixLowPanNdProtocol::AddressRegistration, this);
    }
  else
    {
      ...
      if (reRegistrationTime.IsNegative ())
        {
          // std::cout << Simulator::Now ().As (Time::MS) << " - " << m_node->GetId () << " - calling AddressRegistration (2)" << std::endl;
          m_addressRegistrationEvent = Simulator::Schedule (MilliSeconds (m_addressRegistrationJitter->GetValue ()),
                                                            &SixLowPanNdProtocol::AddressRegistration, this);
        }
      else
...
SixLowPanNdProtocol::AddressRegistrationTimeout ()
{
  ...
  // std::cout << "Address Registration Timeout called for Node ID= " << m_node->GetId () << std::endl;

  NS_LOG_FUNCTION (this);

  NS_ABORT_MSG_IF (m_addressRegistrationEvent.IsRunning (), "Address Registration Timeout but another address registration is in progress.");
  NS_ABORT_MSG_IF (!m_addrPendingReg.isValid,
...
//      std::cout << "***** Registration unsuccessful for " << m_addrPendingReg.addressPendingRegistration << ", trying again " << +m_addressRegistrationCounter << " of " << +m_maxUnicastSolicit << std::endl;
    ...
    // std::cout << Simulator::Now ().As (Time::MS) << " - " << m_node->GetId () << " - calling AddressRegistration (3)" << std::endl;
  ...
  else
    {
      ...
      if (!m_pendingRas.empty ())
        {
          // std::cout << Simulator::Now ().As (Time::MS) << " - " << m_node->GetId () << " - calling AddressRegistration (4)" << std::endl;
          m_addressRegistrationEvent = Simulator::Schedule (MilliSeconds (m_addressRegistrationJitter->GetValue ()),
                                                            &SixLowPanNdProtocol::AddressRegistration, this);
        }
      else
        {
          ...
          if (reRegistrationTime.IsNegative ())
            {
          // std::cout << Simulator::Now ().As (Time::MS) << " - " << m_node->GetId () << " - calling AddressRegistration (5)" << std::endl;
              m_addressRegistrationEvent = Simulator::Schedule (MilliSeconds (m_addressRegistrationJitter->GetValue ()),
                                                                &SixLowPanNdProtocol::AddressRegistration, this);
            }
          else
