...
def trace_cwnd_callback(oldval, newval):
    ...
    cwnd_file.write(
        f"{ns.Simulator.Now().GetSeconds()} {newval / 1448.0}\n"
    )
    cwnd_file.flush()


# C++ wrapper — formatted for clang-format-20
ns.cppyy.cppdef(
    r"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="tcp-bbr-example (Python)"
    )
    parser.add_argument(
        "--tcpTypeId",
        default="TcpBbr",
    ...
    parser.add_argument(
        "--delAckCount", type=int, default=2, help="Delayed ACK count"
    )
    parser.add_argument(
        "--enablePcap", action="store_true", help="Enable pcap traces"
    )
    parser.add_argument(
        "--stopTime", type=float, default=100.0, help="Simulation stop time"
    )
    ...
    ns.Config.SetDefault(
        ...
        ns.QueueSizeValue(ns.QueueSize("1p")),
    )
    ns.Config.SetDefault(
        queueDisc + "::MaxSize",
    ...
    tch.SetQueueLimits(
        "ns3::DynamicQueueLimits", "HoldTime", ns.StringValue("1000ms")
    )
    ...
    throughput_file = open(
        os.path.join(outdir, "py_throughput.dat"), "w"
    )
    queue_file = open(
        os.path.join(outdir, "py_queueSize.dat"), "w"
    )
    cwnd_file = open(os.path.join(outdir, "py_cwnd.dat"), "w")

    if enablePcap:
        bottleneck.EnablePcapAll(
            os.path.join(outdir, "pcap", "bbr"), True
        )
    ...
    ns.Simulator.Schedule(
        ns.Seconds(0.1) + ns.MilliSeconds(1), ev_cwnd
    )
